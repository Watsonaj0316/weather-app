document.addEventListener("DOMContentLoaded", function() {
    ...
    const getWeatherDetails = (cityName, lat, lon) => {
      ...
      fetch(WEATHER_API_URL)
          ...
          .then(data => {
              ...
              // Filter weather data for the next 5 days
              const filteredWeather = list.filter(weatherItem => {
                  const weatherDate = new Date(weatherItem.dt * 1000);
                  return (weatherDate.getTime() >= today) && (weatherDate.getTime() < today + 5 * 24 * 60 * 60 * 1000);
              });
              ...
              for (const date in groupedWeather) {
                  ...
                  const weatherDescription = weatherItems[0].weather[0].description;
  ...
  const fetchForecast = (cityName, lat, lon) => {
      const WEATHER_FORECAST_API_URL = "https://api.openweathermap.org/data/2.5/forecast?lat=" + lat + "&lon=" + lon + "&appid=19b7476c9e106bbc4760a5a8c8ac6547";
